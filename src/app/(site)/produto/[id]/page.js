...
export default async function ProdutoDetalhePage({ params }) {
  ...
  if (!item || item.status !== "ACTIVE") return notFound();
          ...
          <div className="card bg-base-100 shadow">
            <div className="card-body">
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                ...
                {item.size ? (
                  <span className="badge badge-outline">Tam: {item.size}</span>
                ) : null}
                {item.condition ? (
              ...
              <p className="text-xs opacity-70 mt-2">
                *No MVP, esse botão vai abrir o fluxo de proposta/troca.
              </p>
            </div>
          </div>
              ...
              <div className="flex items-center gap-3">
                <div className="avatar">
                  <div className="w-12 rounded-full bg-base-200 flex items-center justify-center">
                    <span className="text-sm">
                      {item.owner?.name?.split(" ").slice(0, 2).map((p) => p[0]).join("")}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm opacity-70">
                    @{item.owner.username || "usuario"}
                  </p>
                </div>

                <Link
                  className="btn btn-sm btn-outline"
                  href={`/avaliar-usuario/${item.owner.id}`}
                >
                  Avaliar
                </Link>
              </div>
